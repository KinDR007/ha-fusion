...
<script lang="ts">
	...
	/** Currently active cell tab (1-based index 1..totalSlots). */
	let activeCellTab = 0;
	...
	/**
	 * Open the Templater modal for a given cell + field.
	 *
	 * Templater writes the Jinja into `sel.template[<key>]` and reads the
	 * initial value the same way. To make this round-trip work exactly like
	 * ButtonConfig (which passes the whole dashboard item to Templater), we
	 * pass the CELL OBJECT ITSELF — not a wrapper around it. The cell needs:
	 *   - a stable `id` so $templates output gets keyed predictably; we
	 *     mint one on first open and write it into the cell.
	 *   - a `template` object so Templater has somewhere to put the script.
	 *
	 * Because we hand Templater the live cell reference, every keystroke in
	 * the editor mutates the same object that lives in `sel.cells`, so the
	 * Jinja persists through close/reopen with no extra glue.
	 */
	function openTemplaterFor(cellIndex: number, key: 'state' | 'color') {
		if (!Array.isArray((sel as any).cells)) (sel as any).cells = [];
		while ((sel as any).cells.length <= cellIndex) (sel as any).cells.push({});
		const cell = (sel as any).cells[cellIndex];
		// Stable id used both by Templater (sel.id) and by FlexGrid runtime
		// when subscribing to render_template results in `$templates[id]`.
		if (!cell.id) {
			cell.id = `${sel?.id ?? 'flex'}_cell_${cellIndex}_${Math.random().toString(36).slice(2, 8)}`;
		}
	...
	function hasTemplate(cellIndex: number, key: 'state' | 'color'): boolean {
...
{#if isOpen}
	<Modal size="large">
		...
		<!-- Row of numbered tabs — one per inner-grid slot -->
		...
		<div class="cell-editor">
			...
			<div class="button-container">
				<!-- Yes is the default (more_info undefined → true) -->
				<button
