<!--
  InfoGridConfig — editor for the info_grid item type. Same shape as
  GridButtonConfig (title + 6 cell rows) but the preview component is
  InfoGrid and each cell exposes only entity_id + label (no per-cell
  icon, since the InfoGrid layout doesn't render per-cell icons).
-->
<script lang="ts">
	...
	function setCellField(index: number, key: 'entity_id' | 'label', value: string | undefined) {
		ensureCells();
